...
void start_game(SOCKET client_socket, const std::string& clientIP, const std::string& username) {
    ...
    // Enviar tablero al servidor inmediatamente
    ...
    // Esperar el primer mensaje TURN| en modo bloqueante
    char buffer[1024] = {0};
    int bytes_received = recv(client_socket, buffer, sizeof(buffer) - 1, 0);
    if (bytes_received <= 0) {
    ...
    // Cambiar a modo no bloqueante para el juego
    u_long mode = 1;
    ...
    // Restaurar socket a modo bloqueante y salir
    mode = 0;
    ioctlsocket(client_socket, FIONBIO, &mode);
            ...
            if (input == "1") {
                try {
                    ...
                    if (response == "ERROR|CONNECTION_LOST") {
                        ...
                        return; // Salir del menú logged-in y volver a login_menu
                    }
                    system("cls");
                    std::cout << "====================\n";
                    ...
                    return; // Salir si hay un error de envío
                }
            } else if (input == "2") {
                ...
                break; // Salir normalmente al cerrar sesión
            } else {
